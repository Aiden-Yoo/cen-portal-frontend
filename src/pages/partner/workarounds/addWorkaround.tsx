...
import {
  createWorkaroundMutation,
  createWorkaroundMutationVariables,
...
const CREATE_WORKAROUND_MUTATION = gql`
  mutation createWorkaroundMutation($input: CreateWorkaroundInput!) {
    createWorkaround(input: $input) {
      ok
      error
      workaround {
        id
...
export const AddWorkaround: React.FC = () => {
  ...
  const onCompleted = (data: createWorkaroundMutation) => {
    const {
      createWorkaround: { ok, error, workaround },
    } = data;
    if (ok) {
      ...
      setUploadedFile([]);
      history.push(`/partner/workarounds/${workaround.id}`);
    } else if (error) {
      notification.error({
        message: 'Error',
  ...
  const [createWorkaroundMutation, { data }] = useMutation<
    createWorkaroundMutation,
    createWorkaroundMutationVariables
  ...
  return (
    <Wrapper>
      <Helmet>
        <title>Add Workaround | CEN Portal</title>
      </Helmet>
      <TitleBar>
        <FormOutlined />
        {' 새 글 등록'}
